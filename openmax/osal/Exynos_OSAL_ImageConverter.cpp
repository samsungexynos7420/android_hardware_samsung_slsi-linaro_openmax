...
typedef OMX_BOOL (*ConvertInitFunc)(const unsigned int width, const unsigned int height, const ALGO_t algo, void **user_data);
    ...
    OMX_U32 nHeight)
{
    ...
    if (pHandle->Init(nWidth, nHeight, HI_JACK, &pHandle->pUserData) != OMX_TRUE) {
    ...
    OMX_PTR                  pBuffer)
{
    ...
    if (pHandle->Run != NULL) {
        pHandle->Run(&inConfig, &outConfig, NULL, max_display_luminance_cd_m2, pHandle->pUserData);
    } else {
        ret = OMX_ErrorBadParameter;
        goto EXIT;
    }
