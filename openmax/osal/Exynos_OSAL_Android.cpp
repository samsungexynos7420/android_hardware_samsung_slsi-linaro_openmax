...
typedef gralloc1_module_t GRALLOC_MODULE;

#define OMX_GRALLOC_USAGE_HW_VIDEO (GRALLOC1_CONSUMER_USAGE_VIDEO_PRIVATE_DATA)
#define OMX_GRALLOC_USAGE_HW_VIDEO_DEC (GRALLOC1_PRODUCER_USAGE_VIDEO_DECODER)  // decoder(prod)
#define OMX_GRALLOC_USAGE_HW_VIDEO_ENC (GRALLOC1_CONSUMER_USAGE_VIDEO_ENCODER)  // encoder(cons)
#define OMX_GRALLOC_USAGE_DISP (GRALLOC1_CONSUMER_USAGE_GPU_TEXTURE)  // decoder(prod)
#define OMX_GRALLOC_USAGE_EXT_AREA (GRALLOC1_PRODUCER_USAGE_PROTECTED_DPB) // decoder(prod)
#define OMX_GRALLOC_USAGE_UVA (GRALLOC1_PRODUCER_USAGE_CPU_READ_OFTEN | GRALLOC1_PRODUCER_USAGE_CPU_WRITE_OFTEN)
#define OMX_GRALLOC_USAGE_LOCK (OMX_GRALLOC_USAGE_UVA | OMX_GRALLOC_USAGE_HW_VIDEO)
#define OMX_GRALLOC_USAGE_PROTECTED (GRALLOC1_PRODUCER_USAGE_PROTECTED)
#else
...
#ifdef GRALLOC_VERSION1
    if ((priv_hnd->producer_usage & OMX_GRALLOC_USAGE_PROTECTED) ||
        (priv_hnd->consumer_usage & OMX_GRALLOC_USAGE_PROTECTED))
#else
    ...
    case OMX_IndexParamGetAndroidNativeBuffer:  /* DEC */
    {
        ...
        if (pExynosComponent->codecType == HW_VIDEO_DEC_SECURE_CODEC) {  // secure decoder component
            ...
            if ((pExynosPort->portDefinition.format.video.nFrameWidth *
                ...
                pANBParams->nUsage |= OMX_GRALLOC_USAGE_EXT_AREA;  /* try to use a CMA area */
            }
        }
    }
        break;
...
void Exynos_OSAL_Performance(EXYNOS_CODEC_TYPE eType, int value, int fps)
{
    using ::android::hardware::power::V1_0::PowerHint;

    if ((eType == HW_VIDEO_ENC_CODEC) ||
        (eType == HW_VIDEO_ENC_SECURE_CODEC)) {
        ...
        if (power != NULL) {
            ...
            power->powerHint(PowerHint::VIDEO_ENCODE, ((value >= 0) && ((value % threshold) == 0))? 1:0);
        }
    }
}
