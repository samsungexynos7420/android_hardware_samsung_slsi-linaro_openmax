    ...
    unsigned int            nAllocLen[MAX_BUFFER_PLANE])
{
    ...
    case (OMX_COLOR_FORMATTYPE)OMX_SEC_COLOR_FormatYVU420Planar:
        if (ePlaneType == PLANE_SINGLE) {
            ...
            nAllocLen[0] = ALIGN(ALIGN(nWidth, 16) * ALIGN(nHeight, 16), 256) + 256;
    ...
    case OMX_COLOR_FormatYUV420Planar16:
        if (ePlaneType == PLANE_SINGLE) {
            ...
            nAllocLen[0] = ALIGN(ALIGN(nWidth * 2, 16) * ALIGN(nHeight, 16), 256) + 256;
            nAllocLen[1] = ALIGN(ALIGN(nWidth * 2, 16) * ALIGN((ALIGN(nHeight, 16) >> 1), CHROMA_VALIGN), 256) + 256;
        }
        break;
    case OMX_SEC_COLOR_FormatYUV420SemiPlanarSBWC:
    case OMX_SEC_COLOR_FormatYVU420SemiPlanarSBWC:
    ...
    case OMX_SEC_COLOR_FormatYUV420SemiPlanarSBWC_L75:
        if (ePlaneType == PLANE_SINGLE) {
            ...
            nAllocLen[0] = SBWCL_8B_Y_SIZE(nWidth, nHeight, 75);    /* SBWC_L50 Y */
            nAllocLen[1] = SBWCL_8B_CBCR_SIZE(nWidth, nHeight, 75); /* SBWC_L50 CBCR */
...
int Exynos_OSAL_DataSpaceToColorSpace(int nDataSpace, int nFormat)
{
...
#ifdef USE_ANDROID
    {
        ...
        if ((property_get("ro.vendor.cscsupported", prop, NULL) <= 0) ||
    ...
    int                            nFormat)
{
    ...
    case CSC_EQ_COLORSPACE_SMPTE170M:
        pColorAspects->nRangeType    = RANGE_LIMITED;
