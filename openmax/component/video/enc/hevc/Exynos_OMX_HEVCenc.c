...
#ifdef USE_ANDROID
        /* If converting RGBA to YUV happens,
         * ColorAsspects is already updated to output port.
         */
        if (!((pVideoEnc->surfaceFormat == (OMX_COLOR_FORMATTYPE)OMX_COLOR_Format32BitRGBA8888) &&
             (pVideoEnc->surfaceFormat == eColorFormat))) {
            Exynos_OSAL_GetColorAspectsForBitstream(&(pInputPort->ColorAspects), &(pOutputPort->ColorAspects));
        }
#endif
        if ((pVideoEnc->surfaceFormat == (OMX_COLOR_FORMATTYPE)OMX_COLOR_Format32BitRGBA8888) ||
            (pVideoEnc->surfaceFormat == (OMX_COLOR_FORMATTYPE)OMX_COLOR_Format32bitARGB8888)) {
            Exynos_OSAL_GetRGBColorTypeForBitStream(&pOutputPort->ColorAspects, pInputPort->ColorAspects.nDataSpace, eColorFormat);
#if 1
            /* HEVC RGB encoding scenario should only supports BT.601.
             * CtsMediaTestCases/HeifWriterTest uses ColorConverter to CSC(YUV->RGB)
             * and this S/W converter only supports BT.601.
             * So restricts color space to BT.601 in condition of HEVC Encoder(for HEIF) && RGB input.
             */
            pOutputPort->ColorAspects.nRangeType    = RANGE_LIMITED;
            pOutputPort->ColorAspects.nPrimaryType  = PRIMARIES_BT601_6_625;
            pOutputPort->ColorAspects.nTransferType = TRANSFER_SMPTE_170M;
            pOutputPort->ColorAspects.nCoeffType    = MATRIX_COEFF_SMPTE170M;
#endif
        }
    ...
    EXYNOS_OMX_DATA     *pSrcInputData)
{
...
#ifdef USE_ANDROID
    {
        ...
        Exynos_OSAL_Performance(pExynosComponent->codecType, pHevcEnc->hMFCHevcHandle.indexTimestamp, fps);
    }
#endif
    ...
    OMX_STRING     componentName)
{
    ...
    pHevcEnc->hMFCHevcHandle.nRefForPframes = 2;
    ...
    Exynos_OSAL_Log(EXYNOS_LOG_ESSENTIAL, "[%p][%s] GetInstInfo for enc : temporal-svc(%d)/roi(%d)/qp-range(%d)/pvc(%d)/I-ratio(%d)/CA(%d)AdaptiveBR(%d)/HDR-ST(%d)/HDR-DY(%d)",
            pExynosComponent, __FUNCTION__,
...
#ifdef USE_ANDROID
    Exynos_OSAL_Performance(pExynosComponent->codecType, 0/* start */, 0);
#endif
...
OMX_ERRORTYPE Exynos_OMX_ComponentDeinit(OMX_HANDLETYPE hComponent)
{
    ...
    Exynos_OSAL_Performance(pExynosComponent->codecType, -1/* end */, 0);
#endif
