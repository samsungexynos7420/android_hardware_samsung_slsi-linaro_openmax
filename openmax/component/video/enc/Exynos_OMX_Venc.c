...
#ifdef USE_ANDROID
        if (!Exynos_OSAL_Strcmp(pExynosComponent->componentName, "OMX.Exynos.HEVC.Encoder")) {
            if ((csc_src_color_format == HAL_PIXEL_FORMAT_EXYNOS_ARGB_8888) ||
                (csc_src_color_format == HAL_PIXEL_FORMAT_RGBA_8888) ||
                (csc_src_color_format == HAL_PIXEL_FORMAT_BGRA_8888)) {
                /* Only supports BT.601 for CtsMediaTestCases/HeifWriterTest.
                 * ColorConverter in libstagefright doesn't support BT.709.
                 * So we restricts color space in condition of HEVC Encoder(for HEIF) && RGB input.
                 */
                colorspace = CSC_EQ_COLORSPACE_SMPTE170M;
            }
        }
#endif
