    ...
    EXYNOS_OMX_DATA     *pSrcInputData)
{
...
#ifdef USE_ANDROID
    {
        ...
        Exynos_OSAL_Performance(pExynosComponent->codecType, pH264Enc->hMFCH264Handle.indexTimestamp, fps);
    }
#endif
    ...
    OMX_STRING     componentName)
{
    ...
    Exynos_OSAL_Log(EXYNOS_LOG_ESSENTIAL, "[%p][%s] GetInstInfo for enc : temporal-svc(%d)/skype(%d)/roi(%d)/qp-range(%d)/fix(%d)/pvc(%d)/I-ratio(%d)/CA(%d)/AdaptiveBR(%d)",
            pExynosComponent, __FUNCTION__,
